...
def helper(dis):
    ...
    pre = [p for p in pre if pd.notna(p) and p != ""]  # Remove NaN or empty values

    med = medications[medications['Disease'] == dis]['Medication']
    med = [med.iloc[0] if not med.empty else "No medication available" for _ in range(len(med) if not med.empty else 1)]

    die = diets[diets['Disease'] == dis]['Diet']
    die = [die.iloc[0] if not die.empty else "No diet recommendation available" for _ in range(len(die) if not die.empty else 1)]

    wrkout = workout[workout['disease'] == dis]['workout']
    wrkout = wrkout.iloc[0] if not wrkout.empty else "No workout recommendation available"
...
def predict():
    if request.method == 'POST':
        ...
        if invalid_symptoms:
            return render_template('index.html', message=f"Invalid symptoms: {', '.join(invalid_symptoms)}. Please use valid symptoms from the list.")

        predicted_disease = get_predicted_value(user_symptoms)
        dis_des, precautions, medications, diets, workout = helper(predicted_disease)

        my_precautions = precautions if precautions else ["No precautions available"]
        medications = medications if medications else ["No medications available"]
        diets = diets if diets else ["No diet recommendations available"]

        return render_template('index.html', predicted_disease=predicted_disease, dis_des=dis_des,
                              my_precautions=my_precautions, medications=medications, diets=diets,
                              workout=workout)
